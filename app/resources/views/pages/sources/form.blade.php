    ...
    <div class="max-w-lg mx-auto">
        ...
        <form method="POST" action="{{ $source->exists ? route('sources.update', $source) : route('sources.store') }}" class="space-y-5">
            ...
            <div>
                ...
                <select name="kind" required class="w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 py-2.5 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500">
            ...
            <div>
                ...
                <textarea name="notes" rows="2" maxlength="500"
                    class="w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 py-2.5 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500">{{ old('notes', $source->notes) }}</textarea>
            </div>
            ...
            <div class="flex gap-3 pt-2">
                ...
                <button type="submit" class="flex-1 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg">{{ $source->exists ? 'Guardar' : 'Crear fuente' }}</button>
            </div>
        </form>
    </div>
</x-app-layout>
